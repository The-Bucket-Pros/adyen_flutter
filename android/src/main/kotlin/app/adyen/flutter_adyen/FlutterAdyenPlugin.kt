        ...
        when (call.method) {
            "openDropIn" -> {
                this.activity?.let { a ->
                    ...
                    val lineItems = call.argument<List<Map<String, String>>>("lineItems")
                    val reference = call.argument<String>("reference")
                    val shopperReference = call.argument<String>("shopperReference")
                    val shopperEmail = call.argument<String>("shopperEmail")
                    val shopperIP = call.argument<String>("shopperIP")
                    val authToken = call.argument<String>("authToken")
                    val merchantAccount = call.argument<String>("merchantAccount")

                    @Suppress("NULLABILITY_MISMATCH_BASED_ON_JAVA_ANNOTATIONS")
                    val lineItemsString = JSONArray(lineItems).toString()
                    @Suppress("NULLABILITY_MISMATCH_BASED_ON_JAVA_ANNOTATIONS")
                    ...
                    try {
                        ...
                        with(sharedPref.edit()) {
                            ...
                            putString("lineItems", lineItemsString)
                            putString("additionalData", additionalDataString)
                            putString("shopperReference", shopperReference)
                            putString("shopperEmail", shopperEmail)
                            putString("shopperIP", shopperIP)
...
class AdyenDropinService : DropInService() {
    ...
    override fun makePaymentsCall(paymentComponentJson: JSONObject): DropInServiceResult {
        ...
        val lineItemsString = sharedPref.getString("lineItems", "UNDEFINED_STR")
        ...
        val shopperEmail = sharedPref.getString("shopperEmail", null)
        val shopperIP = sharedPref.getString("shopperIP", null)
        val uuid: UUID = UUID.randomUUID()
        val reference = sharedPref.getString("reference", uuid.toString())

        var lineItems : List<LineItem>? = null
        log("lineItem: ${lineItemsString}")
        if (lineItemsString?: "[]" != "[]") { // check is not an empty map
            val moshi = Moshi.Builder().add(KotlinJsonAdapterFactory()).build()
            val listType = Types.newParameterizedType(List::class.java, LineItem::class.java)
            val jsonAdapter = moshi.adapter<List<LineItem>>(listType);
            lineItems = jsonAdapter.fromJson(lineItemsString ?: "")
        }
...
fun createPaymentsRequest(
        context: Context, lineItems: List<LineItem>?, paymentComponentData: PaymentComponentData<out PaymentMethodDetails>,
        amount: String, currency: String, reference: String, merchantAccount: String,
        shopperReference: String?, shopperEmail: String?, shopperIP: String?, countryCode: String, additionalData: Map<String, String>
): PaymentsRequest {
